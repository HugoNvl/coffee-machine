...
public class CucumberStepsEspressoCoffeeMachineTest {
    ...
    public EspressoCoffeeMachine coffeeMachine;
    public Mug mug;
    public Cup cup;
    public CoffeeContainer containerWithCoffee;

    @Given("an espresso machine with {double} l of min bean tank capacity, {double} l of max bean tank capacity, {double} l of min water capacity, {double} l of max water capacity, {double} l per h of water flow for the pump")
    public void givenAnEspressoMachine(double minimalBeanTankCapacity, double maximalBeanTankCapacity, double minimalWaterCapacity, double maximalWaterCapacity, double pumpWaterFlow){
        coffeeMachine = new EspressoCoffeeMachine(minimalBeanTankCapacity, maximalBeanTankCapacity, minimalWaterCapacity, maximalWaterCapacity, pumpWaterFlow);
    }

    @And("a {string} with a capacity of {double} l")
    public void withACapacityOf(String containerType, double containerCapacity) {
    ...
    public void plugTheMachineToElectricity() {
        coffeeMachine.plugToElectricalPlug();
    }

    @And("I add {double} l of water in the water tank")
    public void addLitersOfWater(double waterVolume) {
        coffeeMachine.addWaterInTank(waterVolume);
    }

    @And("I add {double} l of {string} in the bean tank")
    public void addLitersOfCoffeeBeans(double beanVolume, String coffeeType) {
        coffeeMachine.addCoffeeInBeanTank(beanVolume, CoffeeType.valueOf(coffeeType));
    }

    @And("I made an espresso {string}")
    public void madeAnEspresso(String coffeeType) throws InterruptedException, CupNotEmptyException, LackOfWaterInTankException, MachineNotPluggedException, CoffeeTypeCupDifferentOfCoffeeTypeTankException, CannotMakeCremaWithSimpleCoffeeMachine {
    ...
    public void theEspressoMachineReturnACoffeeCupNotEmpty() {
        Assertions.assertFalse(containerWithCoffee.isEmpty());
    }


    @And("an espresso volume equals to {double}")
    public void espressoVolumeEqualsTo(double coffeeVolume) {
        assertThat(coffeeVolume, is(containerWithCoffee.getCapacity()));
    }

    @And("an espresso {string} containing a coffee type {string}")
    public void anEspressoCupContainingACoffeeType(String containerType, String coffeeType) {
        if ("mug".equals(containerType))
